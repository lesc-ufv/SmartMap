...
def infer(model_config):
    ...
    for state,dfg_name in states:
        ...
        mapping, max_tree_depths, mcst_simu_flag, num_backtrackings = UtilEval.play_game(config,model,state,environment,config.num_max_expansion_test)
        ...
        mean_visited_rate,mean_visited_nodes,mean_mean_expanded_nodes = UtilEval.calculate_mean_expanded_nodes(model,config,state,environment,config.num_max_expansion_test,config.max_moves) 
